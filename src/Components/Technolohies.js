import React from "react";
import { motion } from "framer-motion";

import { FaReact, FaNode, FaJava, FaBootstrap, FaHtml5, FaCss3 } from "react-icons/fa";
import { FaPhp, FaVuejs, FaAngular, FaLaravel } from "react-icons/fa6";
import { SiExpress, SiMongodb, SiMysql, SiPostman, SiNextdotjs, SiC } from "react-icons/si";
import { BiLogoPostgresql, BiLogoSpringBoot } from "react-icons/bi";
import { RiTailwindCssFill } from "react-icons/ri";
import { AiOutlineJavaScript } from "react-icons/ai";

export default function Technolohies() {
  const iconVariants = (duration) => ({
    initial: { y: -10 },
    animate: {
      y: [10, -10],
      transition: {
        duration,
        ease: "linear",
        repeat: Infinity,
        repeatType: "reverse",
      },
    },
  });

  return (
    <section id="Technolohies" className="w-full pb-20 border-b-[2px] border-b-black">
      <div className="flex flex-col gap-14">

        <h1 className="flex justify-center items-center mt-10 text-6xl font-bold text-white">
          Technologies
        </h1>

        <motion.div
          whileInView={{ opacity: 1, x: 0 }}
          initial={{ opacity: 0, x: -100 }}
          transition={{ duration: 2 }}
          className="flex flex-wrap items-center justify-center gap-4"
        >

          {/* React */}
          <motion.div variants={iconVariants(2.5)} initial="initial" animate="animate"
            className="border-neutral-800 p-4 rounded-2xl border-4">
            <FaReact className="text-6xl text-cyan-400" />
          </motion.div>

          {/* Next.js */}
          <motion.div variants={iconVariants(3)} initial="initial" animate="animate"
            className="border-neutral-800 p-4 rounded-2xl border-4">
            <SiNextdotjs className="text-6xl text-white" />
          </motion.div>

          {/* Vue */}
          <motion.div variants={iconVariants(3.5)} initial="initial" animate="animate"
            className="border-neutral-800 p-4 rounded-2xl border-4">
            <FaVuejs className="text-6xl text-green-500" />
          </motion.div>

          {/* Angular */}
          <motion.div variants={iconVariants(4)} initial="initial" animate="animate"
            className="border-neutral-800 p-4 rounded-2xl border-4">
            <FaAngular className="text-6xl text-red-600" />
          </motion.div>

          {/* Node */}
          <motion.div variants={iconVariants(4.5)} initial="initial" animate="animate"
            className="border-neutral-800 p-4 rounded-2xl border-4">
            <FaNode className="text-6xl text-green-500" />
          </motion.div>

          {/* Express */}
          <motion.div variants={iconVariants(2)} initial="initial" animate="animate"
            className="border-neutral-800 p-4 rounded-2xl border-4">
            <SiExpress className="text-6xl text-white" />
          </motion.div>

          {/* MongoDB */}
          <motion.div variants={iconVariants(2.5)} initial="initial" animate="animate"
            className="border-neutral-800 p-4 rounded-2xl border-4">
            <SiMongodb className="text-6xl text-green-400" />
          </motion.div>

          {/* MySQL */}
          <motion.div variants={iconVariants(3)} initial="initial" animate="animate"
            className="border-neutral-800 p-4 rounded-2xl border-4">
            <SiMysql className="text-6xl text-white" />
          </motion.div>

          {/* PostgreSQL */}
          <motion.div variants={iconVariants(3.5)} initial="initial" animate="animate"
            className="border-neutral-800 p-4 rounded-2xl border-4">
            <BiLogoPostgresql className="text-6xl text-cyan-300" />
          </motion.div>

          {/* PHP */}
          <motion.div variants={iconVariants(4)} initial="initial" animate="animate"
            className="border-neutral-800 p-4 rounded-2xl border-4">
            <FaPhp className="text-6xl text-blue-400" />
          </motion.div>

          {/* Laravel */}
          <motion.div variants={iconVariants(2)} initial="initial" animate="animate"
            className="border-neutral-800 p-4 rounded-2xl border-4">
            <FaLaravel className="text-6xl text-red-500" />
          </motion.div>

          {/* Java */}
          <motion.div variants={iconVariants(2.5)} initial="initial" animate="animate"
            className="border-neutral-800 p-4 rounded-2xl border-4">
            <FaJava className="text-6xl text-cyan-400" />
          </motion.div>

          {/* Spring Boot */}
          <motion.div variants={iconVariants(3)} initial="initial" animate="animate"
            className="border-neutral-800 p-4 rounded-2xl border-4">
            <BiLogoSpringBoot className="text-6xl text-green-500" />
          </motion.div>

          {/* C */}
          <motion.div variants={iconVariants(3.5)} initial="initial" animate="animate"
            className="border-neutral-800 p-4 rounded-2xl border-4">
            <SiC className="text-6xl text-blue-400" />
          </motion.div>

          {/* HTML */}
          <motion.div variants={iconVariants(4)} initial="initial" animate="animate"
            className="border-neutral-800 p-4 rounded-2xl border-4">
            <FaHtml5 className="text-6xl text-orange-500" />
          </motion.div>

          {/* CSS */}
          <motion.div variants={iconVariants(2)} initial="initial" animate="animate"
            className="border-neutral-800 p-4 rounded-2xl border-4">
            <FaCss3 className="text-6xl text-blue-500" />
          </motion.div>

          {/* JavaScript */}
          <motion.div variants={iconVariants(2.5)} initial="initial" animate="animate"
            className="border-neutral-800 p-4 rounded-2xl border-4">
            <AiOutlineJavaScript className="text-6xl text-black bg-yellow-300 rounded" />
          </motion.div>

          {/* Bootstrap */}
          <motion.div variants={iconVariants(3)} initial="initial" animate="animate"
            className="border-neutral-800 p-4 rounded-2xl border-4">
            <FaBootstrap className="text-6xl text-purple-500" />
          </motion.div>

          {/* Tailwind */}
          <motion.div variants={iconVariants(3.5)} initial="initial" animate="animate"
            className="border-neutral-800 p-4 rounded-2xl border-4">
            <RiTailwindCssFill className="text-6xl text-cyan-300" />
          </motion.div>

          {/* Postman */}
          <motion.div variants={iconVariants(4)} initial="initial" animate="animate"
            className="border-neutral-800 p-4 rounded-2xl border-4">
            <SiPostman className="text-6xl text-orange-500" />
          </motion.div>

        </motion.div>
      </div>
    </section>
  );
}
